const bcrypt = require("bcryptjs");
const fs = require("fs");
const User = require("../models/userModel");
const Student = require("../models/studentModel");
const Parent = require("../models/parentModel");
const { generateToken } = require("../utils/jwt");

// helper to append to log
function log(msg) {
  try { fs.appendFileSync("backend/registration.log", msg + "\n"); } catch {};
}

// REGISTER
const registerUser = async (req, res) => {
  try {
    log("registerUser called: " + JSON.stringify(req.body));
    console.log("registerUser called", req.body);
    const { email, password, userType, ...profile } = req.body;

    // basic validation
    if (!email || !password || !userType) {
      console.log("validation failed: missing email/password/userType");
      log("validation failed missing basic fields");
      return res.status(400).json({ message: "Email, password and userType are required" });
    }
    if (!["student", "parent"].includes(userType)) {
      console.log("validation failed: invalid userType", userType);
      return res.status(400).json({ message: "Invalid userType" });
    }

    // check for existing email in users collection
    const existingUser = await User.findOne({ email });
    if (existingUser) {
      console.log("user already exists", email);
      log("user already exists " + email);
      return res.status(400).json({ message: "User already exists" });
    }

    const salt = await bcrypt.genSalt(10);
    const hashedPassword = await bcrypt.hash(password, salt);

    // create base user document
    console.log("creating base user for", email);
    log("creating base user for " + email);
    const user = await User.create({
      email,
      password: hashedPassword,
      userType,
    });
    console.log("base user created", user._id);
    log("base user created " + user._id);

    // create profile depending on type
    if (userType === "student") {
      const {
        studentId,
        firstName,
        middleName = "",
        lastName,
        gender,
        birthdate,
        gradeSection,
      } = profile;

      if (!studentId || !firstName || !lastName || !gender || !birthdate || !gradeSection) {
        // rollback user creation if profile incomplete
        await User.findByIdAndDelete(user._id);
        return res.status(400).json({ message: "All student profile fields are required" });
      }

      // normalize gender to match enum in schema (capitalize first letter)
      const normalizedGender =
        gender.charAt(0).toUpperCase() + gender.slice(1).toLowerCase();

      try {
        console.log("creating student profile for user", user._id, {
          studentId,
          firstName,
          lastName,
          gender: normalizedGender,
        });
        log("creating student profile " + user._id);
        await Student.create({
          userId: user._id,
          studentId,
          firstName,
          middleName,
          lastName,
          gender: normalizedGender,
          birthdate,
          gradeSection,
        });
        console.log("student profile created");
        log("student profile created " + user._id);
      } catch (err) {
        console.error("Error creating student profile:", err);
        log("student create error " + err.message);
        await User.findByIdAndDelete(user._id);
        return res.status(500).json({ message: "Failed to create student profile" });
      }
    } else if (userType === "parent") {
      const {
        firstName,
        middleName = "",
        lastName,
        gender,
        phoneNumber,
      } = profile;

      if (!firstName || !lastName || !gender || !phoneNumber) {
        await User.findByIdAndDelete(user._id);
        return res.status(400).json({ message: "All parent profile fields are required" });
      }

      const normalizedGender =
        gender.charAt(0).toUpperCase() + gender.slice(1).toLowerCase();

      try {
        console.log("creating parent profile for user", user._id, {
          firstName,
          lastName,
          gender: normalizedGender,
          phoneNumber,
        });
        log("creating parent profile " + user._id);
        await Parent.create({
          userId: user._id,
          firstName,
          middleName,
          lastName,
          gender: normalizedGender,
          email, // reference
          phoneNumber,
          children: [],
        });
        console.log("parent profile created");
        log("parent profile created " + user._id);
      } catch (err) {
        console.error("Error creating parent profile:", err);
        log("parent create error " + err.message);
        await User.findByIdAndDelete(user._id);
        return res.status(500).json({ message: "Failed to create parent profile" });
      }
    }

    console.log("sending success response for user", user._id);
    log("sending success response for user " + user._id);
    res.status(201).json({ message: "Account created successfully", user: { id: user._id, email: user.email, userType: user.userType } });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
  }
};

// LOGIN
const loginUser = async (req, res) => {
  try {
    const { email, password } = req.body;

    const user = await User.findOne({ email });
    if (!user) return res.status(400).json({ message: "Invalid email or password" });

    const isMatch = await bcrypt.compare(password, user.password);
    if (!isMatch) return res.status(400).json({ message: "Invalid email or password" });

    const token = generateToken({ id: user._id, email: user.email, userType: user.userType });

    // fetch profile names for response
    let profileData = {};
    if (user.userType === "student") {
      const student = await Student.findOne({ userId: user._id });
      if (student) {
        profileData = {
          firstName: student.firstName,
          middleName: student.middleName,
          lastName: student.lastName,
        };
      }
    } else if (user.userType === "parent") {
      const parent = await Parent.findOne({ userId: user._id });
      if (parent) {
        profileData = {
          firstName: parent.firstName,
          middleName: parent.middleName,
          lastName: parent.lastName,
          children: parent.children || [],
        };
      }
    }

    res.json({
      message: "Login successful",
      token,
      user: { id: user._id, email: user.email, userType: user.userType, ...profileData },
    });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Server error" });
  }
};

module.exports = { registerUser, loginUser };
